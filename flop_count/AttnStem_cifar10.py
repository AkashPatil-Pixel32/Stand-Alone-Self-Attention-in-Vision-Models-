...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, groups=1, base_width=64):
        ...
        self.conv2 = nn.Sequential(
            AttentionConv(width, width, kernel_size=3, padding=1, groups=8),
            nn.BatchNorm2d(width),
            nn.ReLU(),
        )
...
if model_name=='ATNnet50': 
    ...
    if STEM_CONFIG:
        print("Stem is Attention")		
    else:
        print("Stem is Convolution")		
    model = ATNnet50(num_classes=num_classes, stem=STEM_CONFIG)
else:
